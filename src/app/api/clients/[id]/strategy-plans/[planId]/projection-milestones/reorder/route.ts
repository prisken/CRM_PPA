...
import {
  ...
  parseOrderedIds,
  reorderStrategyProjectionMilestones,
...
export async function PUT(
  ...
  const result = await reorderStrategyProjectionMilestones(planId, orderedIds);
  if (result.error) {
    return result.error;
  }
  ...
  await logClientStrategyEvent({
    ...
    action: 'updated',
    label: `Reordered projection milestones on ${planCheck.plan.title}`,
  });

  return NextResponse.json({
    projectionMilestones: result.projectionMilestones.map(
      formatStrategyProjectionMilestone
    ),
  });
}
